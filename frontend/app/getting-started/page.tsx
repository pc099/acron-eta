...
import { getBaseUrlClient, getApiKeyClient } from "@/lib/api";

const CODE_PLACEHOLDER = "YOUR_API_BASE_URL";
...
export default function GettingStartedPage() {
  ...
  useEffect(() => {
    setApiBaseUrl(getBaseUrlClient() || "http://localhost:8000");
    setHasApiKey(!!getApiKeyClient());
  }, []);

  const codeSnippet = `import requests

# Initialize with your API key (from Sign up or Settings)
API_URL = "${apiBaseUrl || CODE_PLACEHOLDER}"
...
print(result["response"], result["cost"])`;
      ...
      <Card className="border-neutral-border bg-neutral-dark p-6 mb-10">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
          ...
          <div className="flex gap-2">
            <Link href={hasApiKey ? "/settings" : "/signup"}>
              <Button variant="primary" size="sm">
                {hasApiKey ? "Manage API key" : "Generate API key"}
              </Button>
            </Link>
            <Link href="/api-docs">
              <Button variant="outline" size="sm" className="border-neutral-border text-white">
                View guide
              </Button>
            </Link>
          </div>
        ...
        <p className="text-neutral-dark-gray mb-4">
          Use your API key to run inference with smart routing and caching. Replace{" "}
          <code className="text-acron-primary_accent">YOUR_API_KEY</code>
          {apiBaseUrl ? (
            <>
              {" "}
              with your key. Base URL is set to your current API:{" "}
              <code className="text-acron-primary_accent">{apiBaseUrl}</code>
            </>
          ) : (
            <>
              {" "}
              and the base URL with your API base (e.g. from Settings).
            </>
          )}
        </p>
  ...
  if (hasApiKey) {
    return (
      <DashboardLayout
        title="Get Started"
        subtitle="Start optimizing inference in minutes"
      >
        <section className="pb-16 px-6">{content}</section>
      </DashboardLayout>
    );
  }
  ...
  return (
    <div className="bg-acron-black text-white min-h-screen">
      <Navbar />
      <section className="pt-28 pb-16 px-6">{content}</section>
    </div>
  );
}
